...
function ToastIcon({ type }: { type: string | undefined }) {
  ...
  if (type === "error") {
    icon = (
      <OctagonXIcon className="text-white" aria-hidden="true" />
    )
  }

  if (type === "loading") {
    icon = (
      <Loader2Icon className="animate-spin text-brand" aria-hidden="true" />
    )
  }
...
function ToastList() {
  ...
  return toasts.map((toastItem) => (
    <Toast key={toastItem.id} toast={toastItem}  className={toastItem.type === "error" ? "error-toast" : undefined}>
      <ToastContent>
